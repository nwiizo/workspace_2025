...
impl Transport for StdioTransport {
    async fn send(&self, message: Message) -> Result<(), Error> {
        ...
        let json = match serde_json::to_string(&message) {
            Ok(s) => s,
            Err(e) => return Err(Error::Serialization(format!("JSON serialization error: {}", e))),
        };
    ...
    match parse_result {
        ...
        Err(e) => {
            ...
            if let Ok(value) = serde_json::from_str::<serde_json::Value>(&processed_json) {
                ...
                if let Ok(msg) = serde_json::from_str::<Message>(&format!("{{\"jsonrpc\":\"2.0\",\"method\":\"unknown\",\"id\":0}}")) {
